...
class BookController extends Controller
{
    ...
    public function store(BookRequest $request){
    	...
    	if($book = Book::create($data)){
    		return redirect()->route('Book.Edit', $book->id)->with(['class'=>'success','message'=>'Thêm Toy thành công.']);
    	}
        else{
        ...
        if($book = Book::find($data['id'])){
            ...
            if($check < 1){
                if($book->delete()){
                    ...
                    return response()->json(['error' => 0, 'message' => 'Xóa Toy thành công']);
                }
            }else{
                return response()->json(['error' => 1, 'message' => 'Toy đang có đơn hàng đang giao']);
            }
        }
        return response()->json(['error' => 1, 'message' => 'Không tìm thấy Toy']);
    }
