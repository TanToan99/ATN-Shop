    ...
    $(".get-book-btt").on('click', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == 1) {
                    let message = new MessageOnTop("Toys was added to cart");
                ...
                if (data == -2) {
                    let message = new MessageOnTop("Maybe this Toy was added");
                    message.Create(message.CONFIG.DANGER_BOOTSTRAP_CLASS);
                    return;
                }
                if (data == -1) {
                    let message = new MessageOnTop("This Toy is not exist");
                    message.Create(message.CONFIG.ALERT_BOOTSTRAP_CLASS);
                    return;
                }
                if (data == -3) {
                    let message = new MessageOnTop("Maximum 5 Toys in once");
                    message.Create(message.CONFIG.ALERT_BOOTSTRAP_CLASS);
                    return;
                }
            },
